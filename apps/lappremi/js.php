...
$('.server-side').DataTable( {
        ...
        "ajax": "apps/lappremi/datarit.php"
    } );
//alert($('#tglon').val());
if($('#tglon').val()){
    //alert($('#tglon').val());
     $('#tablestock').DataTable( {
        ...
        //"ajax": "../server_side/scripts/server_processing.php" NOTE: use serverside script to fatch the data
        "ajax": "apps/lappremi/data.php?tgl="+$('#tglon').val(),
...
$(document).on('click','#detailrh',function(e){
    e.preventDefault();
        ...
        $.post('apps/lappremi/detailrh.php?id='+$(this).attr("data-id"),
                function(html){
                $("#tampilhis").html(html);
                }   
            );
...
function filtermutasi (tg1,tg2){
  $.post('apps/lappremi/detailmutasi.php?tg1='+tg1+'&tg2='+tg2,
                function(html){
                $("#lappremix").html(html);
                }   
            );
...
function trtarget(id){
    window.location.assign("index.php?x=lappremi&tgl="+id);
}
