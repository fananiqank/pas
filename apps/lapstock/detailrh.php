...
<?php
$no=1;
foreach($db->select("(SELECT @rownum:=@rownum+1 norut, a.*,DATE(a.tgl_mtc) as tglshow,b.arm_norangka,b.arm_nopol,b.arm_nolambung FROM tx_maintenance a JOIN m_armada b ON a.arm_id=b.arm_id JOIN (SELECT @rownum:=0) r where a.arm_id='$_GET[id]') a","*") as $val){
	...
	echo "<tr data-toggle='collapse' data-target='#$val[no_mtc]' style='cursor:pointer'>
            ...
            <tr id='$val[no_mtc]' class='collapse'>
                <td colspan=4>$val[keterangan_mtc]</td>
            </tr>";
    $no++;
}
?>      
