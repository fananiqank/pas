...
$columns = array(
	...
	array('db'      => 'nolambung','dt'   => 2, 'field' => 'nolambung',
		   'formatter' => function( $d, $row ) {
	...
	array('db'      => 'arm_id','dt'   => 11, 'field' => 'arm_id',
		   'formatter' => function( $d, $row ) {	
			return "<a href='javascript:void(0)' onclick=\"getEdit('$d')\">Edit</a> | 
					<a href='javascript:void(0)' onclick=\"getLambung('$d')\">No Lambung</a>";
			}
		  ),
...
$joinQuery = "FROM (select a.*,b.arm_merk_name,c.arm_type_name,GROUP_CONCAT(arm_nolambung) nolambung from m_armada a LEFT JOIN m_armada_merk b on a.arm_merk=b.arm_merk_id LEFT JOIN m_armada_type c on a.arm_type=c.arm_type_id group by arm_norangka) as asi";
$extraWhere = "";        
