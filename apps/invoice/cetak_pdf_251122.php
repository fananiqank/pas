...
$pdf->Cell(60, 5, $val[invdtl_uraian], 1, 0, 'C');
$pdf->Cell(60, 5, 'GRAND TOTAL', 1, 1, 'C');
...
foreach($db->select("(select *, @rownum:=@rownum+1 norut from tx_invoice_dtl a JOIN (SELECT @rownum:=0) b where inv_id='$_GET[id]') a","*") as $val){
  ...
  $my_string = $val[invdtl_ritdtl];    
    ...
    foreach ($my_array1 as $key => $value) {
      ...
      {
        $pdf->SetFont('Arial','',10);
        $pdf->Cell(30, 5, date("d-m-Y", strtotime($value[txangkut_tgl])), 1, 0, 'C');
        $pdf->Cell(30, 5, $value[txangkut_ritase], 1, 0, 'C');
        $pdf->Cell(30, 5, $value[txangkut_tonase], 1, 0, 'C');
        $pdf->Cell(30, 5, $value[txangkut_ritase], 1, 0, 'C');
        $pdf->Cell(30, 5, $value[txangkut_tonase], 1, 1, 'R');
    ...
    $pdf->Cell(90, 5, "", 1, 0, 'C');
    $pdf->Cell(30, 5, $jumRit, 1, 0, 'C');
    $pdf->Cell(30, 5, $jumTon, 1, 1, 'R'); 
$num++;
}
...
foreach($db->select("(select *, @rownum:=@rownum+1 norut from tx_invoice_dtl a JOIN (SELECT @rownum:=0) b where inv_id='$_GET[id]') a","*") as $val){
  ...
  $pdf->Cell(40, 5, $val[invdtl_uraian], 1, 0, 'C');
  $pdf->Cell(20, 5, round($val[invdtl_jarak],2), 1, 0, 'C');
  $pdf->Cell(30, 5, $val[invdtl_ritase], 1, 0, 'C');
  $pdf->Cell(20, 5, $val[invdtl_tonase], 1, 0, 'C');
  $pdf->Cell(40, 5, $val[invdtl_uraian], 1, 1, 'C');
...
$pdf->Cell(20, 2, "Senior Manager PT. PAS", 0, 0, 'L');
$pdf->Cell(170, 2, "General Manager PT. RBT", 0, 1, 'R');
