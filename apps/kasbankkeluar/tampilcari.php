...
// echo "select a.*,CONCAT(id_barang,'_',kode_barang,'_',nama_barang,'_',a.id_satuan'_',nama_satuan) as idcon from m_barang a JOIN m_satuan b ON a.id_satuan=b.id_satuan where id_dep = '$_GET[dep]' and id_sub = '$_GET[sub]' and id_kat = '$_GET[kat]'  and id_barang NOT IN (select id_barang from tx_barangmasukdtl where status_brgmasukdtl = 0)";
 foreach($db->select("m_barang a JOIN m_satuan b ON a.id_satuan=b.id_satuan","a.*,CONCAT(id_barang,'_',kode_barang,'_',nama_barang,'_',a.id_satuan,'_',nama_satuan) as idcon","id_barang NOT IN (select id_barang from tx_barangmasukdtl where status_brgmasukdtl = 0)") as $val){
	echo "<option value='$val[idcon]'>$val[kode_barang] - $val[nama_barang] - $val[partnumber_barang]</option>"; 
