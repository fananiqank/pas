...
foreach($db->select("(select a.id_barang, sum(masukmutasi)-sum(keluarmutasi)-(select IFNULL(sum(qty),0) as qty from tx_barangkeluardtl where id_barang = '$_POST[idbarang]' and status_brgkeluardtl = 0) as stok from tx_mutasi a join m_barang b on a.id_barang=b.id_barang where b.id_barang='$_POST[idbarang]' and id_gudang = $_POST[idgudang] and jenisbrg='$_POST[jenis]') as asi","*","")as $val){}
