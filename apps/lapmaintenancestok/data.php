...
$columns = array(
	...
	array('db'      => 'hargajual','dt'   => 3, 'field' => 'hargajual',
		   'formatter' => function( $d, $row ) {
	...
	array('db'      => 'biaya','dt'   => 4, 'field' => 'biaya',
		   'formatter' => function( $d, $row ) {
...
$joinQuery = "FROM (select @rownum:=@rownum+1 no_urut,b.id_barang,d.nama_barang,sum(b.qty_mtcdtl) qty,
concat(substr(c.arm_norangka,-5),' - ',c.arm_nolambung) as gabs,hargajual,(sum(b.qty_mtcdtl) * hargajual) biaya
...
where DATE(tgl_mtc) between '$_GET[tgl1]' and '$_GET[tgl2]' $armada
GROUP BY b.id_barang,d.nama_barang) a ";
$extraWhere = "";        
