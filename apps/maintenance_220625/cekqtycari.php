...
foreach($db->select("(select id_barang, sum(masukmutasi)-sum(keluarmutasi)-(select IFNULL(sum(qty_mtcdtl),0) as qty from tx_maintenancedtl where id_barang = '$_POST[idbarang]' and status_mtcdtl = 0) as stok from tx_mutasi where id_barang='$_POST[idbarang]' and id_gudang = $_POST[idgudang]) as asi","*","")as $val){}
